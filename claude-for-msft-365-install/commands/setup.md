...
Common per-user config for CS teams:

| Carrying | Example |
|---|---|
| Per-CSM Salesforce token | Extension attr — `gateway_token` |
| Per-team Gainsight endpoint | Extension attr — `gateway_url` |
| `mcp_servers`, `skills` by role (CSM vs renewals vs cs-ops) | Bootstrap endpoint |
