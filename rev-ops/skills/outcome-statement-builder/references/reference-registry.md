# Reference Output: Populated Outcome & Value Registry
...
All magnitude values remain [TBD] because the user selected placeholder mode during the
test run. In a production registry, [TBD] entries are replaced with verified customer evidence
before the entry is ratified.
...
| Rationale | The CRO's primary desired outcome is intervention capability — catching deals before they slip. Late-stage slippage reduction feeds quarterly attainment (Stage 4) downstream. |
...
|-------|-------|
| Stage | 4 — Business Goal |
| Rationale | Forecast accuracy at the CRO level is a board and investor metric. Highest-value outcome in the set — most directly tied to CRO credibility and retention economics. |
...
|-------|-------|
| Stage | 3 — Desired Outcome |
| Rationale | Revenue per head (Stage 4) is the goal; scaled rep performance through systematized coaching is the mechanism. Connects directly to P&L at L3. |
...
|-------|-------|
| Stage | 3 — Desired Outcome |
| Rationale | A constraint removal outcome — enables the other five outcomes to operate without exception carve-outs in regulated segments. Full org coverage is the desired state. |
...
| Level | Label | Observable Evidence | Magnitude | CRO/CS Action |
|-------|-------|---------------------|-----------|---------------|
| L0 | Not Achieved | Compliance review not completed; regulated accounts still excluded; data redaction configuration not validated by legal or security | Baseline coverage % captured only | Identify specific blocker: data residency, redaction, or access controls |
| L1 | Partial | Compliance review passed; deployment in progress; at least [TBD]% of previously excluded accounts now covered; full workflow not yet adopted | Coverage % increasing; [TBD] full coverage threshold not yet reached | Confirm data redaction validated by legal before expanding; document clearance artifact |
| L2 | Achieved | [TBD]% of total revenue org covered including previously excluded accounts; same pipeline, coaching, and forecasting workflows running in regulated segment as in standard segment | [TBD]% total org coverage | Document coverage delta; present to CRO as constraint removal proof point |
| L3 | Exceeded | Full org coverage achieved AND deal intelligence from regulated accounts informing CRO-level win/loss analysis — regulated segment no longer a blind spot | Primary coverage metric + regulated account deal intelligence flowing into CRO reporting | Constraint removal story complete; flag for expansion into additional regulated segments |
...
This registry is designed to be parsed by other skills and agents without natural language
processing. Key conventions:

- **Entry IDs** follow `OCV-NNN` format — sequential, unique, stable across updates
- **Status field** is constrained to four values: `Draft | Under Review | Ratified | Deprecated`
- **All table headers** are identical across entries — agents extract by column label
- **Magnitude placeholders** use the pattern `[TBD% — source required]` — parseable as unverified
- **Registry Metadata** YAML block provides machine-readable provenance and schema version
- **Value Chain Stage** field uses the format `Stage N — Label` — the integer N is extractable

When reading this registry programmatically, treat any magnitude field containing `[TBD` as
unverified. Do not surface unverified magnitudes in Sales or CS-facing outputs without flagging.
